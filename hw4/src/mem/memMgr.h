...
class MemRecycleList
{
   ...
   T* popFront() {
      ...
      if(_first!=0){
         ...
         temp=_first;
         void* a=(void*)_first;
         size_t* b=((size_t*)a);   //cast first to size_t*
         _first=(T*)*(b);
         return temp;
      }
      else
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while(temp){
         ...
         MemRecycleList<T>* next =temp->_nextList;
         last=temp;
         temp=next;
      }
      temp=last;
   ...
   T* getMem(size_t t) {
      ...
      bool a;
      a=_activeBlock->getMem(t,ret);
      if(a){
         return ret;
      }
      else{
         size_t te=_activeBlock->getRemainSize();
          te=downtoSizeT(te);
          if(te>S){
          size_t rn =getArraySize(te);
          _activeBlock->getMem(te,ret);
          ...
          _activeBlock=new MemBlock<T>(block,_blockSize);
          #ifdef MEM_DEBUG
          cout << "New MemBlock... " << _activeBlock << endl;
          #endif // MEM_DEBUG
   ...
   size_t getNumBlocks() const {
      ...
      while(temp!=0){
         ++c;
         MemBlock<T>*next=temp;
         temp =next->getNextBlock();
      }
      return c;
   }
};
