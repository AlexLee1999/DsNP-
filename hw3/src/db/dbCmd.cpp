...
{  
   // TODO...
   string options;
   if(!CmdExec::lexSingleOption(option,options,true))
      return CMD_EXEC_ERROR;      
   if (!dbjson.isread){
      cerr<<"Error: DB is not created yet!!"<<endl;
      return CMD_EXEC_ERROR;
   ...
   if(options.size()!=0){
      if(dbjson.exist(options)){
         DBJsonElem s;
         s=dbjson.find(options);
         cout<<"{ "<<s<<" }"<<endl;
      }
      else{
         cerr<<"Error: No JSON element with key "<<options<<" is found."<<endl;
      return CMD_EXEC_ERROR;      
      }
   }
   else{
