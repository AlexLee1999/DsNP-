...
cout<<"==================" << endl;
cout<< "  PI   "  <<right << setw(16) << I << endl;
cout<< "  PO   "  <<right << setw(16) << O << endl;
cout<< "  AIG  "  <<right << setw(16) << A << endl;
cout<< "------------------" << endl;
cout<< "  Total"  <<  right << setw(16) << I+O+A << endl;
}
...
for(int i=0;i<_outputlist.size();i++){
   _outputlist[i]->dfs(gatelist,_gismarked);
}
printdfs(gatelist,true);
}
void
...
CirMgr::writeAag(ostream& outfile) const
{
   ...
   _gismarked++;
   for(int i=0;i<_outputlist.size();i++){
      countdfs(_outputlist[i],_gismarked,counta);
   }
   outfile <<  "aag "  << M << ' ' << I << ' '<< L << ' ' << O << ' ' << counta << endl;
   ...
   for(int i=0;i<_output.size();i++){
      ...
      outfile<<'i'<<i<<' '<<_outputlist[i]->name()<<endl;
      }
   }
   outfile<<'c';
}
...
void
CirMgr::countdfs(CirGate* gate,int marked,int& count)const{
   ...
   for(int i=0;i<gate->faninlist().size();i++){
      if(gate->faninlist()[i]->getType()==AIG_GATE){
         count++;
      }
      countdfs((gate->faninlist()[i]),marked,count);
   }
}
void
